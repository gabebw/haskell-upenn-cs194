...
whatWentWrong messages = foldl addSevereMessageText [] (inOrder (build messages))
    where
        addSevereMessageText texts message
            | isSevereError message = texts ++ [messageText message]
            | otherwise = texts
        ...
        messageText (LogMessage _ _ text) = text
